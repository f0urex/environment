...
time-epoch-convert() {
    local inDate="$1"
    local outDate=$(date -d @"$inDate" --utc "+%F %T.%N")
    echo $outDate
    echo -n $outDate| xclip -sel clip
}
...
# grep with exclude pattern
grepx() {
    if [ $# -ne 2 ]; then
        echo "Usage: grepx FIND_PATTERN EXCLUDE_PATTERN"
        exit
    fi
    grep -r $1 | grep -v $2 | grep $1
}

get-full-path() {
    found=$(find -name "$1")
    full_path=$(readlink -f "$found")
    echo $full_path
    echo -n $full_path | xclip -sel clip
}

# copy to remote serever
# $1 ip
# $2 from
# $3 to
scpu() {
    scp -O $2 "$SSH_OPTS" root@"$1:$3"
}
